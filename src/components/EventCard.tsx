  ...
  return (
    <Wrapper>
      <div className={`group/item relative h-full w-full overflow-hidden ${bgColor ? 'bg-['+bgColor+']' : ''}`} style={{backgroundColor: bgColor || '' }}>
        <div className={`absolute h-full w-full bg-cover bg-center transition-all duration-500 ease-in-out group-hover/item:scale-125`} style={{backgroundImage: `url("${showImage != null ? urlForImage(showImage as unknown as Image).url() : ''}")`}}>
        </div>
